...
/**
 * Represents a disable account transaction that can be applied to the accounts Map.
 */
public class DisableTransaction extends Transaction {
  ...
  public void apply(Map<Integer, Account> accounts) {
    ...
    if (account == null) {
      throw new ViolatedConstraintException(
        "Tried to disable a non-existent account " + accountNumber
      );
    }
    ...
    // Check that the account is not already disabled.
    if (!account.isEnabled()) {
      throw new ViolatedConstraintException(
        "Account " + accountNumber + " is already disabled"
      );
    }
