...
public class TestApply {
  ...
  public void testGoodValues() {
    ...
    Transaction transaction = new DisableTransaction(account.getAccountNumber(), account.getAccountHolder(), 0, "", context);
  ...
  public void testNonExistentAccount() {
    ...
    Transaction transaction = new DisableTransaction(1, "John Doe", 0, "", context);
    ...
    try {
      ...
      assertEquals(
        e.getMessage(), "Tried to disable a non-existent account 1"
      );
    }
  }
  ...
  public void testAlreadyDisabled() {
    ...
    try {
      transaction.apply(accounts);
      fail(
        "Expected ViolatedConstraintException due to account already being disabled"
      );
    } catch (ViolatedConstraintException e) {
      assertEquals(
        e.getMessage(), "Account 1 is already disabled"
      );
    }
  }
  ...
  public void testMismatchedAccountNames() {
    ...
    Transaction transaction = new DisableTransaction(account.getAccountNumber(), "Jane Doe", 0, "", context);
